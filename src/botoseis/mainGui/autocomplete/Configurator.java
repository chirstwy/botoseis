 ...
 */
package botoseis.mainGui.autocomplete;
...
public class Configurator {
    ...
    public static void enableAutoCompletion(final JComboBox comboBox) {
        ...
        final KeyListener keyListener = new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent keyEvent) {
                ...
                if (comboBox.isDisplayable() && !comboBox.isPopupVisible()) {
                    ...
                    if (keyCode == VK_SHIFT || keyCode == VK_CONTROL || keyCode == VK_ALT) {
                        return;
                    }
        ...
        comboBox.addPropertyChangeListener((PropertyChangeEvent e) -> {
            if (e.getPropertyName().equals("editor")) {
                ComboBoxEditor editor1 = comboBox.getEditor();
                if (editor1 != null && editor1.getEditorComponent() != null) {
                    configureTextComponent((JTextComponent) editor1.getEditorComponent(), document, adaptor);
                    editor1.getEditorComponent().addKeyListener(keyListener);
                }
            }
        });
    }
    ...
    public static void configureTextComponent(JTextComponent textComponent, Document document, final AbstractComponentAdaptor adaptor) {
        ...
        if (document.isStrictMatching()) {
            ...
            editorInputMap.put(getKeyStroke(VK_BACK_SPACE, 0), selectionBackwardAction);
            // ignore VK_DELETE and CTRL+VK_X and beep instead when strict matching
            editorInputMap.put(getKeyStroke(VK_DELETE, 0), errorFeedbackAction);
            editorInputMap.put(getKeyStroke(VK_X, CTRL_DOWN_MASK), errorFeedbackAction);
        } else {
            ...
            editorInputMap.put(getKeyStroke(VK_BACK_SPACE, 0), "nonstrict-backspace");
            editorActionMap.put("nonstrict-backspace", new NonStrictBackspaceAction(
                    editorActionMap.get(deletePrevCharAction),
                    editorActionMap.get(selectionBackwardAction),
                    adaptor));
        }
    }
    ...
    static Object errorFeedbackAction = new TextAction("provide-error-feedback") {
        @Override
        public void actionPerformed(ActionEvent e) {
            getLookAndFeel().provideErrorFeedback(getTextComponent(e));
        }
    };
}

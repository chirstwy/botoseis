...
import static botoseis.mainGui.main.MainWindow.NODECOPY;
import botoseis.mainGui.utils.DefaultNode;
import static botoseis.mainGui.utils.DefaultNode.FLOW_TYPE;
import static botoseis.mainGui.workflows.DefaultProcessView.BlockHeight;
import static botoseis.mainGui.workflows.DefaultProcessView.BlockWidth;
...
public class WorkflowView extends javax.swing.JPanel implements java.util.Observer {
    ...
    public void setModel(botoseis.mainGui.workflows.WorkflowModel m) {
        if (m != null) {
            ...
            for (WorkflowProcess w : m.processes()) {
    ...
    public void update(Observable o, Object arg) {
        ...
        for (WorkflowProcess w : wm.processes()) {
            DefaultProcessView pv = new DefaultProcessView(w, m_prmPanel, this);
            add(pv);
            m_procList.add(pv);
    ...
    private void repositionProcesses() {
        ...
        for (int i = 0; i < m_procList.size(); i++) {
            m_procList.get(i).setLocation(px - BlockWidth / 2,
                    py);
            py += BlockHeight + pady;
            if ((py + BlockHeight) > h) {
                py = BlocksStartY;
    ...
    private List<botoseis.mainGui.workflows.DefaultProcessView> m_procList;
    private String m_title;
    private int BlocksStartX = BlockWidth;

...
import static botoseis.mainGui.utils.Utils.getDateFormated;
...
public class ProcessingJobsPanel extends javax.swing.JPanel {
    ...
    private void updateJobsTable() {
        ...
        for (int i = 0; i < m_jobsList.size(); i++) {
            ...
            jobsTable.setValueAt(getDateFormated(m_jobsList.get(i).getTimeStart()), i, 3);
            jobsTable.setValueAt(getDateFormated(m_jobsList.get(i).getTimeStop()), i, 4);
...
private void btnRemoveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRemoveActionPerformed
    int row = jobsTable.getSelectedRow();
    if (row >= 0) {
        if (!jobsTable.getValueAt(row, 7).toString().isEmpty()) {
            java.io.File hd = new java.io.File(m_jobsList.get((parseInt(jobsTable.getValueAt(row, 7).toString()))).getHomeDir());

            int ans = showConfirmDialog(this, "The folder :" + hd.toString()
                    + "\nwill be permanently removed!", "User folder removal", OK_CANCEL_OPTION);
            if (ans == OK_OPTION) {
                deleteFile(hd);
                int id = parseInt(jobsTable.getValueAt(row, 7).toString());
...
private void btnStopActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnStopActionPerformed
    ...
    private List<WorkflowJob> m_jobsList = new ArrayList<>();

...
private void menuSmoothVelocityActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_menuSmoothVelocityActionPerformed
    ...
    if (dlg.isValid()) {
        ...
        int wndA = 1 + (int) Math.floor(wcdp / d2);

        float wt = dlg.getTimeWindow();
        int wndB = 1 + (int) Math.floor(wt / d1);

        int d = dlg.getCdpsInterval();
        int dcdp = (int) (Math.floor(d / d2) * d2);
        float dt = dlg.getTimeInterval();
        float dtime = (float) (Math.floor(dt / d1) * d1);
        ...
        float[][] temp = new float[ncdps][ntime];
